...
class EpisodeData:
    ...
    def to_arrays(self):
        ...
        for attr in ['observations', 'actions', 'rewards', 
                     'sensory_Ia', 'sensory_II', 'sensory_Ib', 
                     'rnn_hidden', 'alpha', 'gamma_static', 
                     'gamma_dynamic', 'target_encoding']:
...
class NetworkDiagram:
    ...
    def __init__(
        ...
        self.norm = Normalize(vmin=-1, vmax=1)
    ...
    def render(
        ...
        radii = {'Ia': 0.15, 'II': 0.15, 'Ib': 0.15, 
                 'rnn': 0.12, 'alpha': 0.15, 'gamma_s': 0.15, 'gamma_d': 0.15,
                 'target': 0.12}
        ...
        for module in modules:
            ...
            # Sanitize activations: replace NaN/Inf with 0, clip to [-1, 1]
            acts = np.nan_to_num(acts, nan=0.0, posinf=1.0, neginf=-1.0)
            ...
            if circles:
                collection = PatchCollection(circles, facecolors=colors, 
        ...
        for i, y in enumerate(muscle_ys):
            self.ax.text(0.2, y, f'M{i+1}', fontsize=8, ha='right', va='center', color='white')
            self.ax.text(11.8, y, f'M{i+1}', fontsize=8, ha='left', va='center', color='white')
        ...
        if title_parts:
            self.ax.set_title(" | ".join(title_parts), fontsize=11, fontweight='bold', color='white')
...
class EpisodeRecorder:
    ...
    def record(
        ...
        env = ReachingEnv(self.xml_path, render_mode=render_mode, 
        ...
        with torch.no_grad():
            for step in range(max_steps):
                # Get target visibility from previous info
                target_visible = info.get('target_visible', False)
                ...
                # Combine frames
                if render_mujoco and render_network and data.mujoco_frames and data.network_frames:
                    combined = self._combine_frames(
                        data.mujoco_frames[-1], 
                        data.network_frames[-1]
                    )
    ...
    def _extract_activations(
        ...
        def sanitize(arr: np.ndarray) -> np.ndarray:
            """Replace NaN/Inf with 0 and clip to [-1, 1]."""
        ...
        if 'rnn_hidden' in net_info:
            data.rnn_hidden.append(sanitize(net_info['rnn_hidden'].squeeze().cpu().numpy()))
        
        # Motor outputs
        if 'alpha' in net_info:
            data.alpha.append(sanitize(net_info['alpha'].squeeze().cpu().numpy()))
        if 'gamma_static' in net_info:
            data.gamma_static.append(sanitize(net_info['gamma_static'].squeeze().cpu().numpy()))
        if 'gamma_dynamic' in net_info:
            data.gamma_dynamic.append(sanitize(net_info['gamma_dynamic'].squeeze().cpu().numpy()))
    ...
    def _get_activations_dict(self, data: EpisodeData, idx: int) -> Dict[str, np.ndarray]:
        """Get activations dict for diagram rendering."""
        return {
            'Ia': data.sensory_Ia[idx] if data.sensory_Ia else np.zeros(self.num_muscles),
            'II': data.sensory_II[idx] if data.sensory_II else np.zeros(self.num_muscles),
            'Ib': data.sensory_Ib[idx] if data.sensory_Ib else np.zeros(self.num_muscles),
            ...
            'gamma_s': data.gamma_static[idx] if data.gamma_static else np.zeros(self.num_muscles),
            'gamma_d': data.gamma_dynamic[idx] if data.gamma_dynamic else np.zeros(self.num_muscles),
            'target': data.target_encoding[idx] if data.target_encoding else np.zeros(16),
        }
...
def record_and_save(
    ...
    - simulation.mp4: MuJoCo only
    - network_activity.mp4: Network diagram only
    ...
    if data.mujoco_frames is not None and len(data.mujoco_frames) > 0:
        video_path = str(output_path / "simulation.mp4")
        _save_video(data.mujoco_frames, video_path, fps)
        print(f"  Saved: {video_path}")
    
    # Save network video
    if data.network_frames is not None and len(data.network_frames) > 0:
        video_path = str(output_path / "network_activity.mp4")
        _save_video(data.network_frames, video_path, fps)
        print(f"  Saved: {video_path}")
...
def plot_episode_summary(
    ...
    if hasattr(data.target_encoding, 'shape') and data.target_encoding.shape[0] > 0:
        ...
        im = ax4.imshow(data.target_encoding.T, aspect='auto', cmap='viridis',
    ...
    # Distance to target
    distances = [info.get('distance_to_target', np.nan) for info in data.infos[1:num_steps+1]]
    ln1 = ax5.plot(time, distances[:num_steps], 'b-', linewidth=2, label='Distance to Target')
    ...
    hand_positions = np.array([info.get('hand_position', [0, 0, 0]) for info in data.infos[1:num_steps+1]])
    ...
    ax5b = ax5.twinx()
    ln2 = ax5b.plot(time, hand_speed[:num_steps], 'r-', linewidth=2, alpha=0.7, label='Hand Speed')
    ...
    ax6.plot(time, energy_penalty[:num_steps], 'r-', linewidth=1.5, label='Energy Penalty', alpha=0.8)
...
if __name__ == '__main__':
    ...
    diagram = NetworkDiagram(num_muscles=4, rnn_hidden_size=128, target_grid_size=4)
