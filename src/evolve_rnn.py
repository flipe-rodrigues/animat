...
class Limb2DMujoco:
    ...
    def sample_target(self):
        ...
        # mujoco.mj_forward(self.model, self.data)
        # self.data.geom_xpos[target_id][:] = target_pos
        return target_pos
    
# Evolutionary Strategy (ES) Optimization
class EvolutionaryLearner:
    def __init__(self, trial_dur=20, num_trials=100, num_individuals=50, num_generations=10, mutation_rate=.1):
    ...
    def init_params(self):
        ...
        return np.random.randn(100 * (15 + 100 + 4)) * 0.1
    ...
    def evaluate(self, params, render=False):
        ...
        for trial in range(self.num_trials):
            ...
            while env.data.time < self.trial_dur:
                ...
                total_reward -= distance  # Closer = better reward
                ...
                # viewer.close()
            # print(f"\tTrial: {trial}, Target: {target}, End position: {env.get_pos()}")
    ...
    def evolve(self):
        ...
        for gen in range(self.num_generations):
            fitnesses = np.array([self.evaluate(p) for p in self.population])
            best_idx = np.argmax(fitnesses)

            print(f"Generation {gen+1}, Best Fitness: {fitnesses[best_idx]:.4f}")

            # Select top individuals
            sorted_indices = np.argsort(fitnesses)[::-1]
            self.population = [self.population[i] for i in sorted_indices[:self.pop_size // 2]]

            # Mutate top performers to create offspring
            new_population = []
            for p in self.population:
                for _ in range(2):  # Each parent creates 2 offspring
                    child = p + np.random.randn(len(p)) * self.mutation_rate
...
if __name__ == "__main__":
    learner = EvolutionaryLearner(trial_dur=5, num_trials=1, num_individuals=50, num_generations=100)
    best_params = learner.evolve()  # Train for 50 generations
