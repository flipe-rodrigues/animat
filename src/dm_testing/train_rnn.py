...
def continue_training_rnn(model_path="arm_rnn_final", vecnorm_path="vec_normalize_rnn.pkl", 
                          additional_steps=400000, new_lr=1.5e-4):
    ...
    # Create a NEW model with the desired learning rate and matching configuration
    new_model = RecurrentPPO(
        "MlpLstmPolicy", 
        vec_normalize,
        learning_rate=new_lr,
        n_steps=2048,          # Match the initial training
        batch_size=256,
        n_epochs=10,
        gamma=0.99,
        gae_lambda=0.95,
...
def evaluate_recurrent_policy(model, env, n_eval_episodes=10, deterministic=True):
    """
    Evaluate a recurrent policy while properly managing LSTM states.
...
class RecurrentEvalCallback(EvalCallback):
    ...
    def _on_step(self):
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            # Use our LSTM-aware evaluation function
            try:
            ...
            # Continue with parent implementation for best model saving
