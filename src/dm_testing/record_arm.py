...
def record_rnn_episodes(num_episodes=5, model_path="./best_model/best_model.zip", 
                    output_path="arm_episodes.gif", stats_path="vec_normalize.pkl"):
    """Record episodes with proper normalization"""
    ...
    # Record each episode with a different seed
    for episode in range(1, num_episodes + 1):
        seed = 30 + episode  # Different seed for each episode
        set_seeds(seed)
        ...
        while not done and step_count < 100:  # Add step limit for safety
            ...
            # Update episode starts for next iteration
...
if __name__ == "__main__":
    ...
    record_rnn_episodes(
        num_episodes=10, 
        model_path="arm_rnn_final_continued.zip", 
        output_path="arm_recurrent_better.gif",
        stats_path="vec_normalize_rnn_continued.pkl"
    )