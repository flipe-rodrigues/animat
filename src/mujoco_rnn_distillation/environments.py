...
from utils import truncated_exponential, l1_norm, l2_norm
...
class RewardCalculator:
    ...
    __slots__ = ("distance_weight", "energy_weight", "ridge_weight", "lasso_weight")

    def __init__(self, loss_weights):
        self.distance_weight = -loss_weights["distance"]
        self.energy_weight = -loss_weights["energy"]
        self.ridge_weight = -loss_weights["ridge"]
        self.lasso_weight = -loss_weights["lasso"]

    def compute(self, distance, energy, ridge=0.0, lasso=0.0):
        """Compute reward using pre-negated weights"""
        return (
            distance * self.distance_weight
            + energy * self.energy_weight
            + ridge * self.ridge_weight
            + lasso * self.lasso_weight
        )
...
class SequentialReachingEnv:
    ...
    def __init__(
        ...
        Args:
            ...
            loss_weights: Dict with 'distance', 'energy', 'ridge', and 'lasso' weights
    ...
    def _validate_config(
        ...
        if not all(k in loss_weights for k in ["distance", "energy", "ridge", "lasso"]):
            raise ValueError(
                "loss_weights must contain 'distance', 'energy', 'ridge', and 'lasso'"
            )
    ...
    def _run_simulation(
        ...
        while target_idx < schedule.num_targets:
            ...
            ridge = l1_norm(rnn.get_params())
            lasso = l2_norm(rnn.get_params())
