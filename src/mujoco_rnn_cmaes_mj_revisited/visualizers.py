...
class SequentialReachingVisualizer:
    ...
    def plot_sensors(self, linewidth=1):
        ...
        # Energy
        axes[2, 0].plot(
            self.data["time"], self.data["energy"], linewidth=linewidth, label="Energy"
        )
        ...
        # Reward / fitness
        axes[2, 1].plot(
            self.data["time"], self.data["reward"], linewidth=linewidth, label="Reward"
        )
        axes[2, 1].set_title("Loss")
        axes[2, 1].set_ylim([-2.05, 0.05])
        ax_right = axes[2, 1].twinx()
        ax_right.plot(self.data["time"], self.data["fitness"], color=(0.25, 0.25, 0.25))
    ...
    def _get_target_onset_times(self):
        ...
        target_onset_idcs = np.where(
            np.any(np.diff(np.array(self.data["target_position"]), axis=0) != 0, axis=1)
        )[0]
