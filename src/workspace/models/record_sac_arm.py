...
def record_sac_episodes_dm_control(num_episodes=5, 
    ...
    # 🔧 CREATE THE SAME ENCODER THAT WAS USED DURING TRAINING
    print(f"Creating encoder for trained model...")
    
    # Import encoder classes
    sys.path.insert(0, str(workspace_root))
    from encoders.encoders import ModalitySpecificEncoder
    
    # Create the SAME encoder used during training
    ...
    # Load normalization stats with CORRECT encoder
    print(f"Loading normalization stats from {vecnorm_path}...")
    
    # Create dummy environment with the SAME encoder
    dummy_env = create_env(random_seed=42, encoder=encoder)  # ✅ Pass encoder!
    dummy_vec = DummyVecEnv([lambda: dummy_env])
    
    # Now VecNormalize should work correctly
    vec_normalize = VecNormalize.load(vecnorm_path, dummy_vec)
    
    obs_mean = vec_normalize.obs_rms.mean  # Shape: (38,) ✅
    obs_std = np.sqrt(vec_normalize.obs_rms.var + vec_normalize.epsilon)  # Shape: (38,) ✅
    ...
    while len(successful_episodes) < num_episodes:
        ...
        # Create environment with SAME encoder
        dm_env = make_arm_env(random_seed=seed)
        raw_env = create_env(random_seed=seed, encoder=encoder, base_env=dm_env)  # ✅ Pass encoder!
...
if __name__ == "__main__":
    ...
    successful_eps, all_eps = record_sac_episodes_dm_control(
        num_episodes=5, 
        model_path="models/arm_final2",  # Your SAC model
        vecnorm_path="models/vec_normalize2.pkl",  # Your normalization stats
        output_path="models/arm_sac_performance.gif",
        fps=20,
