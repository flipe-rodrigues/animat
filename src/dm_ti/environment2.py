...
class ArmReachingTask(composer.Task):
    ...
    def __init__(self, random_state=None):
        ...
        self._goal_threshold = 0.06  # Match your termination condition
        self._target_dwell_count = 0
        self._target_dwell_required = 10  # Require 5 consecutive timesteps at target
    ...
    def initialize_episode(self, physics, random_state):
        ...
        """targets = [
            np.array([0.3, 0.3, 0.0]),   # Top 
            np.array([0.3, -0.3, 0.0]),   # Right
            np.array([-0.5, -0.6, 0.0]),  # Bottom
            np.array([0.5, -0.5, 0.0]),  # Left
        ]
        idx = random_state.randint(0, len(targets))
        target_pos = targets[idx]
        physics.bind(self._arm.target).mocap_pos[:] = target_pos"""

        #target_pos = [0.3, -0.3, 0.0]
        #physics.bind(self._arm.target).mocap_pos[:] = target_pos
        # Set the target orientation (fixed in this case)
    ...
    def get_reward(self, physics):
        ...
        proximity = max(0, 1 - distance / (8 * self._goal_threshold))
        
        # Velocity penalty that scales with proximity to target
        vel_penalty = -0.1 * speed * speed #* proximity**2  # Stronger effect very close to target
        ...
        if distance < self._goal_threshold:
            ...
            if self._target_dwell_count >= self._target_dwell_required and not self._goal_reached:
                sparse += 2.0
                self._goal_reached = True
           
        else:
            # Agent is outside goal threshold
            if self._goal_reached:
                #print("Agent moved away from goal, applying penalty")
                # Agent was at goal but moved away - apply penalty
                penalty = -0.5
                
            
            # Reset dwell count when outside threshold
            self._target_dwell_count = 0
        
            
        
        """if distance < self._goal_threshold and not self._goal_reached:
            self._goal_reached = True
            sparse = 1.5"""
        ...
        #shaping = - 0.5 * distance * distance
        # Combine rewards with annealing
        #reward = sparse + lambda_t * shaping + energy 
        reward = sparse + shaping + energy + penalty #+ vel_penalty#+ proximity_bonus + penalty #+ energy #+ distance_penalty #+ stability_bonus + vel_penalty # Use this for debugging
        
        # Increment step counter
        self._step_count += 1
        
        flag = False
        if int(physics.data.time / self.control_timestep) % 100 == 0 and flag:
    ...
    def should_terminate_episode(self, physics):
        ...
        if (int(physics.data.time / self.control_timestep) >= MIN_STEPS and 
                ...
                self._target_dwell_count >= self._target_dwell_required): 
            return True
        return False
...
def make_arm_env(random_seed=None):
    ...
    # Create task with the random state
    task = ArmReachingTask(random_state=random_state)
    
    # Create environment with the same random state
    env = composer.Environment(
        task=task,
        time_limit=2.0,
        random_state=random_state,  # Pass the same random state
