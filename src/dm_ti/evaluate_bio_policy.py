...
def get_args():
    parser = argparse.ArgumentParser(description='Evaluate a trained bio-policy and record video')
    parser.add_argument('--checkpoint', type=str, default='models/policy.pth', 
                        help='Path to the checkpoint file (best reward was at epoch 200)')
    parser.add_argument('--video-dir', type=str, default='videos', 
                        help='Directory to save videos')
    parser.add_argument('--video-name', type=str, default='arm_reaching2.gif',  # Changed to .gif
                        help='Output video filename')
    parser.add_argument('--num-episodes', type=int, default=3, 
                        help='Number of episodes to evaluate')
    parser.add_argument('--hidden-size', type=int, default=128, 
                        help='Hidden layer size (must match training)')
    parser.add_argument('--tau-mem', type=float, default=16.0, 
                        help='Membrane time constant (must match training)')
    parser.add_argument('--tau-adapt', type=float, default=70.0, 
                        help='Adaptation time constant (must match training)')
...
def main():
    ...
    print(f"Creating policy with observation dim {state_shape[0]} and action dim {action_shape[0]}")
    ...
    for episode in range(args.num_episodes):
        ...
        while not (terminated or truncated):
            ...
            action = result.act[0]  # Get first action from batch
            state = result.state    # Update recurrent state
            ...
            # Track hand/target positions on EVERY step (moved outside conditional)
            ...
            # Print stats only every 20 steps
            if timesteps % 20 == 0:
